...
app.post("/v1/responses", async (req, res) => {
  ...
  try {
    const response = await axios.post( // gửi yêu cầu post tới api của gpt
    ...
    const output = response.data.choices.map((choice) => ({ // Lấy nội dung từ gpt gửi về
      content: [{ text: choice.message.content }], // Định dạng lại nội dung
    }));

    res.json({ output }); // Gửi phản hồi về người dùng
  } catch (error) { // bắt lỗi, in ra màn hình
